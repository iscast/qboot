...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
        ...
        ,table = {
            ...
            ,addRow:function (tableId, index, data) {
                ...
                if(options.isTree) {
                    ...
                    if(uo) {
                        that.treeNodeOpen(uo, true, index);//展开节点
            ...
            * 给数据添加叶子节点
            * */
            ,addChildren: function(tableId, id, data) {
            	if (!table.children[tableId]) table.children[tableId] = {};
            	if (!table.children[tableId][id]) table.children[tableId][id] = {};
                table.children[tableId][id].children = data;
            }
            /**
             * 删除一行或多行数据
             * （如果是树状则删除自己和子节点）
    ...
    Class.prototype.resetData=function(list) {
        ...
        for (var i = 0; i < list.length; i++) {
            var n = list[i];
            if(options.isTree && list[i].hasSub > 0){
    ...
    Class.prototype.treeConvertShowName=function (o) {
        ...
        var isTreeNode=(o.children&&o.children.length>0) || o.type < 2;
     ...
     */
    Class.prototype.treeNodeOpen=function (o,is_open, _index) {
        var that = this
            ,options = that.config
        ...
        if(o.is_open){//打开状态
            if( o.hasSub > 0 && o.children.length === 0 && !!options.getMenu ) {
                options.getMenu(o.id, _index + 1);
            }
            dbClickI.html('&#xe625;');
        }else{
            dbClickI.html('&#xe623;');
    ...
    Class.prototype.events = function(){
        ...
        that.elem.on('click', 'i.layui-tree-head', function(){
            ...
            var o=datas[index];
            that.treeNodeOpen(o,!o.is_open, index);
        });
...
//单元格编辑
        ...
        that.layBody.on('click', 'tr', function(){
            ...
            var allRows = othis.parent().parent()[0].firstElementChild.childElementCount;
            for(var id=0; id<allRows; id++){
            	var tr = that.layBody.find('tr[data-index="'+ id +'"]')
            ...
            var  options = that.config;
            var activeData = table.getDataList(options.id)[index];
            var flag = false;
            if( !!table.children[options.id] && !!table.children[options.id][activeData.id] && !!table.children[options.id][activeData.id].children) {
            	flag = true;
            }
            if(activeData.hasSub == 0 ) {
                if(!!options.getButtons && activeData.type != options.buttonType ) {
                    if( !flag ) {
                        options.getButtons( activeData.id, index + 1, activeData.id );
                    } else {
                        options.getButtons( table.children[options.id][activeData.id].children, index + 1, activeData.id );
                    }
                }
            }else{
            	options.getButtons(null, index + 1 ,activeData.id);
            }
        });
    ...
    //自动完成渲染
    table.init();
    layui.link('module/treeGrid/treeGrid.css');//引入css
